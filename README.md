# Hi 👋, I'm Anshika Lohan

B.Tech 3rd year CSE @ Manipal University Jaipur (CGPA: 9.25)
AI | Machine Learning | Gen AI | Full Stack Developer

---

## About Me

* Passionate about **AI, ML, and Real-world Problem Solving**
* Strong in **Data Analysis, Computer Vision, NLP, Generative AI & RAG Systems**

---

## Tech Stack

### Languages

`C` `C++` `Python` `Java` `SQL` `JavaScript`

### AI/ML

`TensorFlow` `Scikit-learn` `OpenCV` `LangChain`

### Development

`Flask` `FastAPI` `Streamlit` `React` `HTML` `CSS`


### Databases

`MySQL` `MongoDB` `PostgreSQL`

---

## GitHub Stats

![Streak](https://streak-stats.demolab.com?user=anshikalohan\&theme=tokyonight\&hide_border=true)

---

## Achievements

* 1st Place – MUJ ACM Hackathon
* Dean’s List (4 Semesters)
* SIH 2025 – Selected among 500+ teams

---

## Experience

**Salesforce Launchpad Trainee @ PwC**

* Training in **Salesforce, Gen AI, Data Systems & CRM**

---


## Connect With Me

* [anshika.lohan.03@gmail.com](mailto:anshika.lohan.03@gmail.com)
* LinkedIn: https://linkedin.com/in/anshikalohan

![Profile Views](https://komarev.com/ghpvc/?username=anshikalohan&label=Profile%20Views&color=0e75b6&style=flat)
